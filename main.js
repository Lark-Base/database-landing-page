...
const TEMPLATE_COPY_FEEDBACK_MS = 2000;
const AGENT_REDIRECT_DELAY_MS = 2000;
...
// 数据行第 0 项为维度名，后续索引与列标题对应。
const DATABASE_COLUMN_INDEX = COMPARISON_COLUMNS.length;
const COMPARISON_CARD_COLUMN_ORDER = [
  DATABASE_COLUMN_INDEX,
  ...COMPARISON_COLUMNS.slice(0, -1).map((_, index) => index + 1),
];
...
function initTableFade(scope = document) {
  for (const el of scope.querySelectorAll('.table-scroll')) {
    ...
    const onScroll = () => {
      ...
      clearTimeout(timer);
      timer = setTimeout(() => el.classList.remove('is-scrolling'), SCROLL_IDLE_DELAY_MS);
    };
...
function initHeroCta() {
  ...
  const bindCta = (cta) => {
    ...
    cta.addEventListener('click', async () => {
      if (cta.classList.contains('is-busy')) {
        return;
      }
      ...
      if (!isFooter) {
        ...
        for (let i = 0; i < LOADING_DOT_COUNT; i += 1) {
      ...
      setTimeout(
        ...
        isFooter ? COPY_FEEDBACK_MS : AGENT_REDIRECT_DELAY_MS,
      );
    });
  };
  ...
  if (copyBtn) {
    ...
    copyBtn.addEventListener('click', async () => {
      ...
      clearTimeout(timer);
      timer = setTimeout(() => copyBtn.classList.remove('is-copied'), TEMPLATE_COPY_FEEDBACK_MS);
    });
  }
}
...
function initTemplates() {
  ...
  pre.addEventListener(
    ...
    () => {
      ...
      scrollTimer = setTimeout(() => pre.classList.remove('is-scrolling'), SCROLL_IDLE_DELAY_MS);
    },
    { passive: true },
  );
