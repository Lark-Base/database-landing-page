...
function stepFluid(sim, dt, cfg) {
  ...
  const {
    velX, velY, velX0, velY0,
    density, density0, pressure, divergence,
  ...
  if (cfg.project) project(velX, velY, pressure, divergence, cfg.projectIterations, w, h);
  ...
  if (cfg.densityDissipation > 0) {
    ...
    for (let i = 0; i < size; i += 1) density[i] *= decay;
  }
}
...
function splatRing(sim, cx, cy, force, densityAmount, radius, thickness, randomness, seed, scaleX, scaleY) {
  ...
  if (force <= 0 && densityAmount <= 0) return;
  ...
  for (let py = y0; py <= y1; py += 1) {
    ...
    for (let px = x0; px <= x1; px += 1) {
      ...
      if (dens > 0) density[i] = saturate(density[i] + dens * falloff);
    }
  }
}
...
function buildAtlas(atlasCanvas, atlasCtx, layout, charset, fill) {
  if (!layout.tileW || !layout.tileH || charset.length === 0) return false;
  ...
  const scaleW = maxW > 0 ? (0.92 * innerW) / maxW : 1;
...
function computeCellMetrics({ fontSize, charWidth, cellAspectRatio = DEFAULT_ASPECT, cellPadding }) {
  const size = Number.isFinite(fontSize) ? fontSize : (Number.isFinite(charWidth) ? charWidth * cellAspectRatio : 14 * cellAspectRatio);
  const glyphH = Math.max(1, size);
  const glyphW = Math.max(1, Number.isFinite(size / cellAspectRatio) ? size / cellAspectRatio : 14);
...
function computeFitRect({ sourceWidth, sourceHeight, destWidth, destHeight, fit, position }) {
  ...
  if (mode === 'contain') {
    ...
    return { sx: 0, sy: 0, sw: sourceWidth, sh: sourceHeight, dx: (destWidth - dw) * px, dy: (destHeight - dh) * py, dw, dh };
  }
  if (mode === 'fill') {
  ...
  if (mode === 'none') {
    return {
      ...
      dx: (destWidth - sourceWidth) * px, dy: (destHeight - sourceHeight) * py,
  ...
  return {
    sx: (sourceWidth - sw) * px, sy: (sourceHeight - sh) * py, sw, sh,
    dx: 0, dy: 0, dw: destWidth, dh: destHeight,
  };
}
...
export function createAsciiTrail(container, options = {}) {
  ...
  ctx.font = prevFont;
  const probeAscent = Number.isFinite(probe.actualBoundingBoxAscent) ? probe.actualBoundingBoxAscent : 80;
  const probeDescent = Number.isFinite(probe.actualBoundingBoxDescent) ? probe.actualBoundingBoxDescent : 20;
  const measuredAspect = (probeAscent + probeDescent) / (probe.width || 60);
  const cellAspectRatio = Number.isFinite(measuredAspect) && measuredAspect > 0 ? measuredAspect : DEFAULT_ASPECT;

  const base = computeCellMetrics({
    fontSize: cfg.fontSize,
    charWidth: cfg.charWidth ?? 14,
  ...
  const mapLuma = (() => {
    ...
    return (luma) => {
      let v = luma;
      if (cfg.contrast !== 1) v = (v - 0.5) * cfg.contrast + 0.5;
      v = saturate(v);
      if (cfg.gamma !== 1) v = v ** cfg.gamma;
      v = saturate(v);
  ...
  let layout = {
    widthCss: 0, heightCss: 0, drawWidth: 0, drawHeight: 0,
    offsetX: 0, offsetY: 0, cols: 0, rows: 0, dpr: 1,
    tileW: 0, tileH: 0,
    cellWidth: base.cellWidth, cellHeight: base.cellHeight,
    drawCellWidth: base.drawCellWidth, drawCellHeight: base.drawCellHeight,
    padX: base.padX, padY: base.padY,
  };
  ...
  const setState = (state) => {
    if (canvas.dataset.asciiOverlayState !== state) canvas.dataset.asciiOverlayState = state;
  };

  const refreshAtlas = (color) => {
    const fill = color ?? window.getComputedStyle(canvas).color;
    if (buildAtlas(atlasCanvas, atlasCtx, layout, charset, fill)) atlasColor = fill;
  };
  ...
  const resize = () => {
    ...
    const changed = layout.widthCss !== wCss || layout.heightCss !== hCss
      || layout.dpr !== dpr || layout.cols !== cols || layout.rows !== rows;

    layout = {
      ...layout,
      widthCss: wCss, heightCss: hCss,
      drawWidth: wCss, drawHeight: hCss,
      offsetX: 0, offsetY: 0,
      cols, rows, dpr,
      tileW: Math.max(1, Math.round(drawCellWidth * dpr)),
      tileH: Math.max(1, Math.round(drawCellHeight * dpr)),
      cellWidth, cellHeight, drawCellWidth, drawCellHeight, padX, padY,
    };
  ...
  const ensureSim = () => {
    ...
    if (!sim || sim.width !== w || sim.height !== h) {
      ...
      sim.density.fill(0); sim.density0.fill(0);
      sim.velX.fill(0); sim.velY.fill(0);
      sim.velX0.fill(0); sim.velY0.fill(0);
      sim.pressure.fill(0); sim.divergence.fill(0);
      needsFluidClear = false;
    }
    return sim;
  };
  ...
  const advanceSplashes = (s, now, dtMs) => {
    ...
    for (let i = splashes.length - 1; i >= 0; i -= 1) {
      ...
      const lifeMs = (maxRadius / speed) * 1000;
      if (elapsed > lifeMs) {
        splashes.splice(i, 1);
        continue;
      }
      ...
      const force = fluid.forceScale * fluid.splashForceScale * fluid.splashVelocityScale * forceDecay * 20;
  ...
  const render = (now) => {
    ...
    try {
      ...
      if (color !== atlasColor) refreshAtlas(color);
      ...
      if (cols !== layout.cols || rows !== layout.rows) {
        layout = { ...layout, cols, rows, drawWidth: fitRect.dw, drawHeight: fitRect.dh, offsetX: fitRect.dx, offsetY: fitRect.dy };
        sampleCanvas.width = cols;
        sampleCanvas.height = rows;
      } else {
      ...
      if (!lumaBuf || !atlasCanvas.width || !atlasCanvas.height) return;
      ...
      const smoothFactor = smoothing
        ? (!Number.isFinite(dtMs) || dtMs <= 0 ? 0 : saturate(1 - Math.exp(-dtMs / cfg.lumaSmoothingMs)))
        : 0;
      ...
      const { cellWidth, cellHeight, drawCellWidth, drawCellHeight, padX, padY, tileW, tileH, offsetX, offsetY } = layout;
      ...
      for (let y = 0; y < rows; y += 1) {
        ...
        for (let x = 0; x < cols; x += 1) {
          ...
          if (f <= 0) continue;
          if (f < 1) luma = cfg.invertLuma ? luma * f : luma * f + (1 - f);

          const glyph = mapLuma(luma);
          if (glyph === spaceIndex && spaceIndex !== -1) continue;

          ctx.drawImage(
            atlasCanvas,
            glyph * tileW, 0, tileW, tileH,
  ...
  const onPointerMove = (e) => {
    if (!fluidEnabled || fluid.strength <= 0) return;
    const rect = container.getBoundingClientRect();
    ...
    const dist = Math.hypot(dx, dy);
    const dt = pointerPrev ? clamp((now - pointerPrev.time) / 1000, 0.001, 0.05) : 0.016;
  ...
  const onPointerDown = (e) => {
    if (e.isPrimary === false || !fluidEnabled) return;
    const rect = container.getBoundingClientRect();
    const lx = e.clientX - rect.left;
    const ly = e.clientY - rect.top;
  ...
  const onPointerUp = (e) => {
    if (e.isPrimary === false || activePointerId !== e.pointerId) return;
    const wasClick = !dragging;
    activePointerId = null;
    pointerDownAt = null;
    dragging = false;
    if (!wasClick || !fluidEnabled || fluid.splashRangePx <= 0) return;
    ...
    const gx = lx - layout.offsetX;
    const gy = ly - layout.offsetY;
    if (gx < 0 || gy < 0 || gx > layout.drawWidth || gy > layout.drawHeight) return;
    ...
    const corner = Math.max(
      Math.hypot(gx, gy), Math.hypot(gx, ry),
      Math.hypot(rx, gy), Math.hypot(rx, ry),
    );
    ...
    splashes.push({
      ...
      maxRadiusPx,
      thicknessPx: fluid.splashThicknessPx > 0
        ? fluid.splashThicknessPx
        : Math.max(8, Math.min(80, 0.08 * maxRadiusPx)),
    });
  };
  ...
  resize();
  refreshAtlas();
  if (!source.complete) source.addEventListener('load', resize, { once: true });
